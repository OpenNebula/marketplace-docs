...
Once the interface list is provided to the VNF, the service deployed inside the Virtual Router starts to monitor OpenNebula via OneGate for changes to the IP address allocations. Esp., to the **NIC aliases assigned** to any VM on any of virtual networks connected to the Virtual Router instance. Based on the aggregated data, it builds a list of pairs for SNAT/DNAT where the destination part is the IP address of the NIC alias and the source part is the real IP address assigned to the VM we try to reach.

To hot-attach a **NIC alias** to the existing VM, we must use CLI tools and pass a template file (e.g.: ``nic-alias.tmpl``) with content similar to the one below (network name, ID and NIC must be adapted to your use-case):
...
.. prompt:: bash $ auto
   ...
   $ onevm nic-attach 10 --file nic-alias.tmpl
