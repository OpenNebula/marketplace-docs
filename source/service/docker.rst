...
* No default login (local or SSH) password - must be provided via `contextualization <https://docs.opennebula.io/stable/management_and_operations/references/kvm_contextualization.html#user-credentials>`_.
...
Follow the instructions above to download and install the appliance. When the appliance is downloaded a template with the same name is created. It's recomended to update the template with a network to make the vm accessible from the outside. Additionally you can adjust the disk size, set the root password or the ssh key etc.

You can also create a template by hand and set the appliance image there.
